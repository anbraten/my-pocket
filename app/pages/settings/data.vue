  ...
  <div class="space-y-6">
    <UiCard class="space-y-4">
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          >
          <UiSelect
            v-model="selectedImportAccountId"
      ...
      />
      <p v-if="importing" class="text-xs text-stone-500 dark:text-stone-400">
      ...
      >
        <span v-if="importedCount > 0"
          >{{ importedCount }} transactions imported.</span
        >
        <span v-if="importedCount > 0 && taggedCount > 0"> </span>
        <span v-if="taggedCount > 0"
          >{{ taggedCount }} existing transactions tagged to account.</span
        >
        <span v-if="importedCount === 0 && taggedCount === 0"
          >No new transactions found.</span
        >
      </div>
      <div
        v-if="importError"
    ...
    <UiCard class="space-y-4">
      <header>
        ...
        <p class="text-sm text-stone-500 dark:text-stone-400">
          Export all your data or restore from a previous backup. Everything
      ...
      <div class="grid gap-6 sm:grid-cols-2">
        <div class="space-y-2">
          ...
          <p class="text-xs text-stone-500 dark:text-stone-400">
            Saves all transactions and accounts as a JSON file.
          </p>
        </div>
        <div class="space-y-2">
          <p
          ...
          />
          <p class="text-xs text-stone-500 dark:text-stone-400">
            Replaces all existing data with the selected backup.
          </p>
        </div>
      </div>
      <p v-if="restoring" class="text-xs text-stone-500 dark:text-stone-400">
        Restoring…
      </p>
      <div
        v-if="restoreError"
...
const handleRestoreUpload = async (event: Event) => {
  ...
  try {
    ...
    const txs = backup.transactions.map((t: any) => ({
      ...t,
    ...
    await db.transaction(
      ...
      async () => {
        ...
        if (backup.categoryModel?.length)
          await db.categoryModel.bulkAdd(backup.categoryModel);
      },
    );
