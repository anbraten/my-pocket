<template>
  <div class="space-y-6">
    <UiCard class="space-y-4">
      <header>
        ...
        <p class="text-sm text-stone-500 dark:text-stone-400">
          Retrain on everything you've labeled and fill in all uncategorized
          transactions.
        </p>
      </header>
      ...
      >
        {{
          recategorizeResult === 0
            ? 'No uncategorized transactions found.'
            : `${recategorizeResult} transaction${recategorizeResult === 1 ? '' : 's'} categorized.`
        }}
      </div>
    </UiCard>
    ...
    <UiCard variant="danger">
      <header class="mb-4">
        <p
          class="text-xs uppercase tracking-wider text-red-400 dark:text-red-300"
        >
          Danger zone
        </p>
...
const { clearRecurringData } = useRecurring();
